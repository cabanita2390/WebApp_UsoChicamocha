<script>
  ...
  import {
    monitoringMotoDocumentColumns,
    monitoringMotoOilColumns,
  ...
  /** Intervalos habituales en motos (menor que livianos); por defecto 3.000 km. */
  const MOTO_OIL_INTERVAL_DEFAULT = 3000;

  let oilForm = {
    oilType: '',
    brandId: null,
    kmAtChange: null,
    intervalKm: MOTO_OIL_INTERVAL_DEFAULT,
  ...
  async function handleRefresh() {
    await data.fetchMotoMonitoring();
  }
  ...
  async function openDocModal(row) {
    ...
    try {
      ...
      if (docVehicleId == null) throw new Error('No se encontró la motocicleta.');
      docModalOpen = true;
    } catch (e) {
      addNotification({
        id: Date.now(),
        text: e.message || 'No se pudo cargar la moto para actualizar documentos.',
      });
      actionRow = null;
    }
  }
  ...
  <div class="vehicle-module-inner">
    ...
    {#if isLoading}
      <div class="vehicle-loader">
        <Loader />
        <p>Cargando monitoreo de motos...</p>
      </div>
    ...
    {:else}
      <div class="vehicle-table-wrap vehicle-table-wrap--inset">
        {#if activeTab === 'documentos'}
          <DataGrid
            columns={monitoringMotoDocumentColumns}
            data={motos}
            totalElements={motos.length}
            totalPages={1}
            currentPage={0}
            pageSize={Math.max(motos.length, 1)}
        ...
        {:else}
          <DataGrid
            columns={monitoringMotoOilColumns}
            data={motos}
            totalElements={motos.length}
            totalPages={1}
            currentPage={0}
  ...
  <div class="modal-overlay" role="presentation" on:click={resetModals}>
    <div class="modal-box modal-box-wide" role="dialog" aria-modal="true" on:click|stopPropagation>
      ...
      <div class="modal-grid">
        <label>
          Km en el cambio
          <input type="number" bind:value={oilForm.kmAtChange} disabled={modalSubmitting} placeholder="Ej: 12000" />
        </label>
        <label>
          Tipo / viscosidad
        ...
        <label>
          Intervalo hasta próximo cambio (km)
